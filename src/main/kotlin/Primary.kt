 ...
 *
 * @see <a href="https://leetcode.cn/leetbook/read/top-interview-questions-easy/xngt85/">Fizz Buzz</a>
 */
 ...
 *
 * @see <a href="https://leetcode.cn/leetbook/read/top-interview-questions-easy/xnzlu6/">计数质数</a>
 */
fun countPrimes(n: Int): Int {
    // 埃氏筛
    // 初始化只是质数数组,并填写默认值为1
    var primes = IntArray(n,{1})
    // 计数器
    var count = 0
    var i = 2
    var j = 0L
    ...
    while(i < n){
        if(primes[i] == 1){
            count++
            // 从 i*i 开始，所有i的倍数，都置为合数，即0
            // i*i 开始防止重复，比如 2*6 和 6*2
            // i的倍数一定是合数，j += i 就等于i的倍数在递增
            // 此处注意，i*i 可能会超过Int的MAX_VALUE，所以先转成Long。
            j = i.toLong() * i.toLong()
            while(j < n){
                // 作为下标，再转回来Int。
                primes[j.toInt()] = 0
 ...
 *
 * @see <a href="https://leetcode.cn/leetbook/read/top-interview-questions-easy/xnsdi2/">3的幂</a>
 */
 ...
 * 给定一个罗马数字，将其转换成整数。
 *
 * @see <a href="https://leetcode.cn/leetbook/read/top-interview-questions-easy/xn4n7c/">罗马数字转整数</a>
 */
